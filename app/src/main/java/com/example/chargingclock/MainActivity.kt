...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Инициализация музыкального блока
    ...
    private fun updateMusicUI() {
        ...
        if (duration > 0) {
            seekBarMusic.max = duration.toInt()
        }
    ...
    // --- ИСПРАВЛЕННАЯ ФУНКЦИЯ: Всегда включаем фильтр MULTIPLY для календаря ---
    private fun applyRedText() {
        ...
        // Применяем фильтр для всех тем. Используем MULTIPLY для лучшего контраста.
    ...
    private fun requestLocationUpdate() { if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) { ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.ACCESS_FINE_LOCATION), 100); return }; tvWeatherCondition.text = "Поиск..."; val lastLocation = locationManager.getLastKnownLocation(LocationManager.NETWORK_PROVIDER); if (lastLocation != null) { lat = lastLocation.latitude.toString(); lon = lastLocation.longitude.toString(); weatherUpdateRunnable.run() }; locationManager.requestSingleUpdate(LocationManager.NETWORK_PROVIDER, object : LocationListener { override fun onLocationChanged(location: Location) { lat = location.latitude.toString(); lon = location.longitude.toString(); fetchWeather() }; override fun onStatusChanged(provider: String?, status: Int, extras: Bundle?) {}; override fun onProviderEnabled(provider: String) {}; override fun onProviderDisabled(provider: String) {} }, null) }
    private fun fetchWeather() { val prefs = getSharedPreferences("AppConfig", Context.MODE_PRIVATE); val apiKey = prefs.getString("YANDEX_API_KEY", ""); if (apiKey.isNullOrEmpty()) { CoroutineScope(Dispatchers.Main).launch { tvWeatherCondition.text = "Введите ключ"; tvWeatherTemp.text = "--"; tvWeatherIcon.text = "" }; return }; CoroutineScope(Dispatchers.IO).launch { try { val urlString = "https://api.weather.yandex.ru/v2/forecast?lat=$lat&lon=$lon&lang=ru_RU"; val url = URL(urlString); val connection = url.openConnection() as HttpURLConnection; connection.setRequestProperty("X-Yandex-Weather-Key", apiKey); if (connection.responseCode == 200) { val data = connection.inputStream.bufferedReader().readText(); val json = JSONObject(data); val fact = json.getJSONObject("fact"); val temp = fact.getInt("temp"); val condition = fact.getString("condition"); val conditionRu = translateCondition(condition); val icon = getWeatherIcon(condition); withContext(Dispatchers.Main) { tvWeatherTemp.text = "${if(temp > 0) "+" else ""}$temp°"; tvWeatherCondition.text = conditionRu; tvWeatherIcon.text = icon } } else { withContext(Dispatchers.Main) { if (connection.responseCode == 403) tvWeatherCondition.text = "Ошибка ключа" else tvWeatherCondition.text = "Err: ${connection.responseCode}" } } } catch (e: Exception) { withContext(Dispatchers.Main) { tvWeatherCondition.text = "Нет сети" } } } }
    ...
    private fun translateCondition(cond: String): String { return when(cond) { "clear" -> "Ясно"; "partly-cloudy" -> "Малооблачно"; "cloudy" -> "Облачно с проясн."; "overcast" -> "Пасмурно"; "drizzle" -> "Морось"; "light-rain" -> "Небольшой дождь"; "rain" -> "Дождь"; "moderate-rain" -> "Умеренный дождь"; "heavy-rain" -> "Сильный дождь"; "showers" -> "Ливень"; "wet-snow" -> "Дождь со снегом"; "light-snow" -> "Небольшой снег"; "snow" -> "Снег"; "hail" -> "Град"; "thunderstorm" -> "Гроза"; else -> cond } }
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) { super.onRequestPermissionsResult(requestCode, permissions, grantResults); if (requestCode == 100 && grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) { requestLocationUpdate() } else { weatherUpdateRunnable.run() } }
